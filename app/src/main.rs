...
fn App() -> Element {
    ...
    let direction_hint = if !show_preview && !has_launched {
        let v = current.flow_at(current.a.0, current.a.1, k());
        let speed = v.length();
        if speed > 1e-6 {
            let hint_len = 0.9;
            let (ux, uy) = (v.x / speed, v.y / speed);
            Some((
                current.a.0,
                current.a.1,
                current.a.0 + ux * hint_len,
                current.a.1 + uy * hint_len,
            ))
        } else {
            None
        }
            ...
            div { class: "panel",
                ...
                svg { view_box: "0 0 {W} {H}", class: "field-svg",
                    ...
                    if let Some((sx, sy, ex, ey)) = direction_hint {
                        line {
                            x1: "{map_x(sx):.2}", y1: "{map_y(sy):.2}",
                            x2: "{map_x(ex):.2}", y2: "{map_y(ey):.2}",
